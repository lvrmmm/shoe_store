...
public class ProductListFrame extends JFrame {
    ...
    private AtomicBoolean isEditFormOpen = new AtomicBoolean(false);
    ...
    private void loadSuppliersForFilter() {
        if (supplierFilterCombo == null) return;
    ...
    private void applyFiltersAndRefresh() {
        ...
        String selectedSupplier = supplierFilterCombo != null ? (String) supplierFilterCombo.getSelectedItem() : "Все поставщики";
        ...
        if (selectedSupplier != null && !selectedSupplier.equals("Все поставщики")) {
            filtered = filtered.stream()
                    .filter(p -> p.getSupplier().getName().equals(selectedSupplier))
                    .collect(Collectors.toList());
        }
    ...
    private JPanel createProductCard(Product product) {
        ...
        if (discount > 0) {
            ...
            String priceHtml = "<html>Цена: <font color='red'><strike>" +
                    priceFormat.format(product.getPrice()) + " ₽</strike></font>" +
    ...
    private void openAddProductForm() {
        if (isEditFormOpen.get()) {
            JOptionPane.showMessageDialog(this,
                    "Окно редактирования уже открыто. Закройте его перед созданием нового товара.",
                    "Внимание",
    ...
    private void openEditProductForm(Product product) {
        if (isEditFormOpen.get()) {
            JOptionPane.showMessageDialog(this,
                    "Окно редактирования уже открыто. Закройте его перед редактированием другого товара.",
                    "Внимание",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
