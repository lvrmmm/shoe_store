...
public class ProductService {
    ...
    public Optional<Product> getProductByArticle(String article) {
     ...
     * Формат: 1 буква + 3 цифры + 1 буква + 1 цифра (A112T4)
     * Генерируется следующий по порядку после максимального существующего
    ...
    private String generateArticle() {
        ...
        while (attempts < maxAttempts) {
            if (allProducts.isEmpty()) {
                ...
                String maxArticle = allProducts.stream()
                ...
                newArticle = String.format("%c%03d%c%d",
                        firstLetter, numberPart, secondLetter, lastDigit);
            }
    ...
    /**
     * Поиск товаров по тексту (по всем текстовым полям)
     * Используется в ProductListFrame для фильтрации
     */
    public List<Product> searchProducts(String searchText) {
        ...
        return getAllProducts().stream()
                .filter(p ->
                        p.getArticle().toLowerCase().contains(lowerSearch) ||
     ...
     * Используется в ProductListFrame
     */
    public List<Product> filterBySupplier(List<Product> products, String supplierName) {
        if (supplierName == null || supplierName.equals("Все поставщики")) {
            return products;
        }

        return products.stream()
                .filter(p -> p.getSupplier().getName().equals(supplierName))
                .collect(Collectors.toList());
    }
     ...
     * Используется в ProductListFrame
     */
    public List<Product> sortByQuantity(List<Product> products, boolean ascending) {
        return products.stream()
                .sorted((p1, p2) -> {
                    int q1 = p1.getQuantity() != null ? p1.getQuantity() : 0;
                    int q2 = p2.getQuantity() != null ? p2.getQuantity() : 0;
                    return ascending ? Integer.compare(q1, q2) : Integer.compare(q2, q1);
                })
                .collect(Collectors.toList());
    }

    /**
     * Получить список всех поставщиков для фильтра
     * Используется в ProductListFrame
     */
